...
deprecatedChain.chain = function(obj){
  ...
  if(!_this._queue){
    ...
    _this._queue.unshift = function (data, callback) {
      var _this = this;
  ...
  for (var k in _this) {
    if ((typeof _this[k] === 'function') && k !== "chain") {
...
deprecatedChain.next = function(){
  this._queue.unshift(
      {name: arguments[0], args: Array.prototype.slice.call(arguments)});
};
  ...
  patch: function(browser) {
    for (var key in browser) {
      if (typeof browser[key] === 'function') {
        browser[key] = deprecatedChain[key].bind(browser);
      }
    }
  }
};
