...
webdriver.prototype._request = function(httpOpts, cb, attempts) {
  var _this = this;
  _this.emit('http', 'Connecting with: ' + JSON.stringify(_this._httpConfig));
  request(httpOpts, function(err, res, data) {
    ...
    if( _this._httpConfig.retries >= 0 &&
      ...
      _this.emit('http', 'Lost http connection, retrying in' + _this._httpConfig.retryDelay + 'ms');
      setTimeout(function() {
...
webdriver.prototype.configureHttp = function() {
  ...
  var cb = fargs.callback,
      opts = fargs.all[0];
  _(this._httpConfig).intersection(opts).each(function(val, key) {
    switch(key) {
      case 'timeout':
