...
webdriver.prototype._newError = function(opts)
{
  ...
  err.inspect = function() {
    ...
    _.each(this, function(v, k) {
      if (typeof v === 'object') {
        var vAsStr;
        if (v["class"] &&  v["class"].match(/org.openqa.selenium.remote.Response/)) {
          ...
          vAsStr = JSON.stringify(v, function(key, value) {
            ...
            if (key === 'screen' || key === 'stackTrace' || key === 'buildInformation' || key === 'localizedMessage') {
              displayedValue = '[hidden]';
            } else if (key === 'message') {
              // trying to extract browser error message
              var messageMatch = value.match(/([^\n]+)\nCommand duration/);
              if((messageMatch) && (messageMatch.length >= 1)) { browserError = messageMatch[1].trim(); }
            }
        ...
        if((data === '') || (data === 'OK')) {
          ...
          if ((jsonWireRes) && (jsonWireRes.sessionId) && (jsonWireRes.status !== undefined)) {
            ...
            if(jsonWireRes.status === 0) {
              ...
              cb(_this._newError(
                {message:'Error response status: ' + jsonWireRes.status +  '.',status:jsonWireRes.status,cause:jsonWireRes} ) );              
            }
          } else {         
            // something wrong
            cb(_this._newError(
    ...
    res.on('end', function() {
      ...
      if (obj.status > 0) {
        var err = _this._newError(
          {message:'Error response status: ' + obj.status + '.',status:obj.status,cause:obj});
        var jsonwireError  = _this._getJsonwireError(obj.status);
        if(jsonwireError){ err['jsonwire-error'] = jsonwireError; }
        cb(err);
      } else {
        cb(null, obj);
      }
...
webdriver.prototype._jsonWireCall = function(opts) {
  ...
  // set our http inactivity timeout at the time connections are made
  if (this._httpInactivityTimeout) {
    req.setTimeout(this._httpInactivityTimeout);
    req.on('timeout', function() {
      // inactivity timeout hit!  let's tear down this connection
      if (req && req.connection && typeof req.connection.end === 'function') {
        req.connection.end();
      }
    });
  }
