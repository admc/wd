...
var Webdriver = module.exports = function(configUrl) {
  ...
  for (var key in config.httpConfig) {
    this._httpConfig[key] = config.httpConfig[key];
  }
};
...
Webdriver.prototype._init = function() {
  ...
  if(desired.deviceName || desired.device || desired.wdNoDefaults ||
    ...
    // using default
    for (key in this.defaultCapabilities) {
      if (_desired[key] === undefined) {
        _desired[key] = this.defaultCapabilities[key];
      }
    }
  }
    ...
    var cb = function(err) {
      if(err) {
        ...
        _this.emit('command', "RESPONSE" , name + niceArgs(fargs.all),
          niceResp(Array.prototype.slice.call(cbArgs)));
