...
Webdriver.prototype._request = function(httpOpts, cb, attempts) {
  ...
  request(httpOpts, function(err, res, data) {
    ...
    if( _this._httpConfig.retries >= 0 &&
      ...
      if(err) {
        _this.emit('http', 'error code:', + err.code , 'detail:', err);
      }
      cb(err, res, data);
    }
  });
...
Webdriver.prototype.init = function() {
  ...
  var desired = fargs.all[0] || {},
      sessionID = desired.sessionID;

  // keep this around for the request callback
  this._initCallback = fargs.callback;

  if (sessionID) {
    this.sessionID = sessionID;
    this._jsonWireCall({ method: 'GET', cb: this._initCB.bind(this) });
  } else {
    ...
    this._request(httpOpts, this._initCB.bind(this));
  }
};

Webdriver.prototype._initCB = function(err, res, data) {
    var cb = this._initCallback;
    ...
    if(!this.sessionID){
      ...
      try{
        ...
        this.sessionID = locationArr[locationArr.length - 1];
      } catch(ignore){}
    }

    if (this.sessionID) {
      this.emit('status', '\nDriving the web on session: ' + this.sessionID + '\n');
      if (cb) { cb(null, this.sessionID, resData); }
    } else {
      data = strip(data);
      if (cb) {
