...
var webdriver = module.exports = function(configUrl) {
  ...
  this._httpConfig = _.clone(http.httpConfig);
};
...
webdriver.prototype.chain = function(obj){
  if(this.warnDeprecated && !deprecationMessageShown.chain) {
    deprecationMessageShown.chain = 1;
...
webdriver.prototype.setHttpTimeout = function() {
  if(this.warnDeprecated && !deprecationMessageShown.setHttpTimeout) {
    deprecationMessageShown.setHttpTimeout = 1;
    console.warn("setHttpTimeout/setHTTPInactivityTimeout has been deprecated, use configureHttp instead.");
  }
...
webdriver.prototype.configureHttp = function() {
  ...
  var cb = fargs.callback,
      opts = fargs.all[0];
  http._configureHttp(this._httpConfig, opts);
  if(cb) { cb(null); }
};
...
webdriver.prototype._waitForVisible = function(using, value, timeout, pollFreq) {
  if(this.warnDeprecated && !deprecationMessageShown.waitForVisible) {
    deprecationMessageShown.waitForVisible = 1;
    console.warn("waitForVisible has been deprecated, use waitForElement + isVisible asserter instead.");
  }
  var cb = findCallback(arguments);
...
webdriver.prototype.waitForJsCondition = function(){
  if(this.warnDeprecated && !deprecationMessageShown.waitForJsCondition) {
    deprecationMessageShown.waitForJsCondition = 1;
    console.warn("waitForJsCondition has been deprecated, use waitFor + jsCondition asserter instead.");
  }
