...
// WdConfig is an option object containing the following fields, all optional:
// - host: the IP of the Selenium server to connect to. Defaults to the loopback IP.
// - port: the port of the Selenium server to connect to. Defaults to 4444, the default Selenium serer port.
// - username: if using a service that requires authentication, this will be used along the `accessKey` field to provide HTTP authentication headers.
// - accessKey: if using a service that requires authentication, this will be used along the `username` field to provide HTTP authentication headers.
var webdriver = module.exports = function(remoteWdConfig) {
  EventEmitter.call( this );
  this.sessionID = null;
  var auth = remoteWdConfig.auth && remoteWdConfig.auth.split(':'); // accept [Node's `url.parse`](http://nodejs.org/docs/v0.10.0/api/url.html) syntax
  this.username  = remoteWdConfig.username  || (auth && auth[0]) || process.env.SAUCE_USERNAME;
  this.accessKey = remoteWdConfig.accessKey || (auth && auth[1]) || process.env.SAUCE_ACCESS_KEY;
  this.basePath = (remoteWdConfig.path || '/wd/hub');
  this.https = (remoteWdConfig.https || false);
  element = remoteWdConfig.element || require('./element').element;
  // default
  this.options = {
    host: remoteWdConfig.hostname || remoteWdConfig.host || '127.0.0.1'
...
webdriver.prototype._callbackWithData = function(cb) {
  ...
  return _this._callbackWithDataBase(function(err,obj) {
    ...
    if(obj.value !== null && typeof obj.value.ELEMENT !== "undefined") {
        obj.value = new element(obj.value.ELEMENT, _this);
    } else if (toString.call(obj.value) === "[object Array]") {
        for (var i = 0; i < obj.value.length; i++) {
            if (obj.value[i] !== null && typeof obj.value[i].ELEMENT !== "undefined") {
                obj.value[i] = new element(obj.value[i].ELEMENT, _this);
            }
        }
    }
    cb(null, obj.value);
 ...
 */
webdriver.prototype.windowSize = function(win, width, height, cb) {
this._jsonWireCall({
  method: 'POST'
...
/**
  ...
  */
webdriver.prototype.isEnabled = function(element, cb) {
  this._jsonWireCall({
    method: 'GET'
 ...
 */
webdriver.prototype.equalsElement = function(element, other, cb) {
  this._jsonWireCall({
    method: 'GET'
 ...
 */
webdriver.prototype.uploadFile = function(filepath, cb) {
  var fs = require('fs')
    , path = require('path');

  var zip = require('node-zip')();

  zip.file(path.basename(filepath), fs.readFileSync(filepath, 'utf8'));
