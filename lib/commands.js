...
/**
 * broadcastIntent(intent, cb) -> cb(err)
 *
 * @jsonWire POST /session/:sessionId/appium/broadcast_intent
 */
commands.broadcastIntent = function() {
  var fargs = utils.varargs(arguments);
  var cb = fargs.callback,
      netspeed = fargs.all[0];
  ...
  this._jsonWireCall({
    method: 'POST'
    , relPath: '/appium/device/broadcast_intent'
    , data: data
    , cb: simpleCallback(cb)
  });
};
