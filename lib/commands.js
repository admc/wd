...
commands.safeExecute = _safeExecute;
...
var _eval = function(code) {
  ...
  code = this._codeToString(code);
  code = "return " + code + ";";
  _execute.apply(this, [code, function(err, res) {
    if(err) {return cb(err);}
    cb(null, res);
  }]);
};
 ...
 * @jsonWire POST /session/:sessionId/execute
 */
var _safeEval = function(code) {
  var cb = findCallback(arguments);
  code = this._codeToString(code);
  _safeExecute.apply(this, [code, function(err, res) {
    if(err) {return cb(err);}
    cb(null, res);
  }]);
};
commands.safeEval = _safeEval;
...
commands.executeAsync = _executeAsync; 
 ...
 * @jsonWire POST /session/:sessionId/execute_async
 */
var _safeExecuteAsync = function() {
  var fargs = utils.varargs(arguments);
  var cb = fargs.callback,
      code = fargs.all[0],
      args = fargs.all[1] || [];
...
commands.safeExecuteAsync = _safeExecuteAsync;
...
commands.windowName = function() {
  var cb = findCallback(arguments);
  // jshint evil: true
  _eval.apply(this, ["window.name", cb]);
};
...
commands.waitForElement = _waitForElement;

var _waitForVisible = function(using, value, timeout, pollFreq) {
  deprecator.warn('waitForVisible',
    'waitForVisible has been deprecated, use waitForElement + isVisible asserter instead.');

  var cb = findCallback(arguments);

  _waitForElement.apply(this, [using, value, asserters.isVisible, timeout, pollFreq, function(err, isVisible) {
...
commands.waitForVisible = _waitForVisible;
...
var addMethodsForSuffix = function(type, singular, plural) {
  if(singular){
    ...
    commands['waitForElement' + utils.elFuncSuffix(type)] = function() {
      var args = __slice.call(arguments, 0);
      args.unshift(utils.elFuncFullType(type));
      _waitForElement.apply(this, args);
    };

    commands['waitForVisible' + utils.elFuncSuffix(type)] = function() {
      var args = __slice.call(arguments, 0);
      args.unshift(utils.elFuncFullType(type));
      _waitForVisible.apply(this, args);
    };
...
commands.waitForConditionInBrowser = function() {
  ...
  // calling script
  _this.safeExecuteAsync.apply( _this, [_waitForConditionInBrowserJsScript,
 ...
 */
commands.sauceJobUpdate = function(jsonData, done) {
  var _this = this;
