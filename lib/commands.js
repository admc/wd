...
/**
 * getElementScreenshot(element, cb) -> cb(err)
 *
 * @jsonWire POST /session/:sessionId/element/:id/screenshot
 */
commands.getElementScreenshot = function(element) {
  var cb = findCallback(arguments);
  this._jsonWireCall({
    method: 'GET'
...
/**
 * saveElementScreenshot(path, cb) -> cb(err, filePath)
 *
 ...
 */
commands.saveElementScreenshot = function() {
  var _this = this;
  var cb = findCallback(arguments);
  var fargs = utils.varargs(arguments);
  ...
  buildFilePath(_path, function(err, filePath) {
    commands.getElementScreenshot.apply(_this, [function(err, base64Data) {
      if(err) { return cb(err); }
