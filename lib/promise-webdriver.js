...
// gets an array of names of the functions on the given object
...
function filterPromisedMethods(Obj) {
  return functionsIn(Obj).filter(function(fname) {
...
module.exports = function(WebDriver, Element, chainable) {
  ...
  filterPromisedMethods(WebDriver.prototype).forEach(function(fname) {
    PromiseWebdriver.prototype[fname] = wrap(WebDriver.prototype[fname], fname);
  });

  // wrapping element methods with promises.
  filterPromisedMethods(Element.prototype).forEach(function(fname) {
  ...
  PromiseWebdriver.prototype._enrich = function(obj, currentEl) {
    ...
    if(utils.isPromise(obj) && !obj.__wd_promise_enriched) {
      ...
      var allPromisedMethods =
        promisedMethods.slice().concat(
          elementPromisedMethods.filter(
            function(value) {return promisedMethods.indexOf(value) === -1;}));

      // adding browser + element methods to the current promise.
      allPromisedMethods.forEach(function(fname) {
        promise[fname] = function() {
          ...
          return this.then(function(res) {
            ...
            var isBrowserMethod =
              promisedMethods.indexOf(fname) >= 0;
            var isElementMethod =
              el && elementPromisedMethods.indexOf(fname) >= 0;
            ...
            if(isElementMethod) {
              ...
              return el[fname].apply(el, args).then(function(res) {
                if(elementChainableMethods.indexOf(fname) >= 0) {
