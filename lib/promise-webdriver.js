    ...
    _(allPromisedMethods).each(function(fname) {
      promise[fname] = function() {
        ...
        return this.then(function(res) {
          ...
          var el;
          // check if an element context is present in __wd_el_ctx.
          // if it is a case we unwrap the result and current element scope.
          if(res && res.__wd_el_ctx){
            el = res.__wd_el_ctx;
            res = res.res;
          }
          ...
          if(isElementMethod){
            // element method case.
            var nextRes = Q.when(el[fname].apply(el, args))
              .then(function(res) {
                // we wrap the result to preserve the element scope in the
                // next call
                return {
                  __wd_el_ctx: el,
                  res: res
