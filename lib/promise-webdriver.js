...
module.exports = function(WebDriver, chainable) {
  ...
  function enrich(obj, browser){
    ...
    if(isPromise(obj) && !obj.__wd_promise_enriched) {
      ...
      var allPromisedMethods = _.union(
        filterPromisedMethods(browserProto), 
        browserProto._Element? filterPromisedMethods(browserProto._Element.prototype) : []
      );
      ...
      _(allPromisedMethods).each(function(fname) {
        promise[fname] = function() {
          ...
          return this.then(function(res) {
            ...
            if(res instanceof element) {el=res;}
            ...
            if(isBrowserMethod && isElementMethod){
              ...
              } else if(fname.match(/element/) || args[0] instanceof element) {
  ...
  // we get the list of methods first cause we need it in the enrich method.
  var promisedMethods = filterPromisedMethods(WebDriver.prototype);
  var elementPromisedMethods = filterPromisedMethods(element.prototype);
  var allPromisedMethods = _.union(promisedMethods, elementPromisedMethods);

  // making the method list visible
  promiseWebdriver.promisedChainMethods = allPromisedMethods;
  ...
  // adding the enrich method to transfer promiseness
  promiseWebdriver.prototype._enrich = function(target){
    return enrich(target, this);
  }
  promiseElement.prototype._enrich = function(target){
    return enrich(target, this.browser);
  }
