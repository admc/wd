var utils = require("./utils"),
    ...
    ImageElement = require("./image-element"),
...
exports.imageElementCallback = function(cb, browser) {
  cb = cb || cbStub;
  return callbackWithDataBase(function(err, obj) {
    if (err) return cb(err);
    if(isWebDriverException(obj.value)) {return cb(newError(
    ...
    if (!obj.value.rect) {
      cb(newError(
        {message:"no rect in response value field.",cause:obj}));
    } else {
      var el = new ImageElement(obj.value.rect, browser);
      cb(null, el);
    }
  });
};
