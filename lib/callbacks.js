    ...
    if((data === '') || (data === 'OK')) {
      ...
      if (jsonWireRes && (jsonWireRes.sessionId) && (jsonWireRes.status !== undefined)) {
        ...
        if(jsonWireRes.status === 0) {
          ...
          var error = newError(
            { message:'Error response status: ' + jsonWireRes.status + ': ' + jsonwireError.summary + ', ' + jsonwireError.detail
...
var callbackWithDataBase = function(cb) {
  ...
  return function(err, data) {
    ...
    if (obj.status > 0) {
      ...
      var error = newError(
        { message:'Error response status: ' + obj.status + ': ' + jsonwireError.summary + ', ' + jsonwireError.detail + ' ' + alertText
          , status:obj.status
          , cause:obj });
      if(jsonwireError){ error['jsonwire-error'] = jsonwireError; }
      cb(error);
