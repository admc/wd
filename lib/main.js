...
function addPromisedMethod(name, method, opts) {
  opts = opts || {};
  var enrichedMethod = function() {
    var args = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
    var promise = new Q(method.apply(this, args));
    this._enrich(promise);
    return promise;
  };
  if(!opts.chainOnly) { PromiseWebdriver.prototype[name] = method; }
  PromiseChainWebdriver.prototype[name] = enrichedMethod;
}
...
module.exports = {
  ...
  rewrap: function() {
    ...
    'rewrap has been deprecated, use addAsyncMethod/addPromisedMethod instead.');  
    wrap();
  },

  // http
  configureHttp: http.configureHttp,
  ...
  addPromisedMethod: addPromisedMethod,
