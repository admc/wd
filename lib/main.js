...
webdriver.prototype.title = function(cb) {
    ...
    var req = http.request(
        _this.getOpts({url:'/title', method: "GET"}),
        function(res) {
            res.setEncoding('utf8');
...
webdriver.prototype.text = function(element, cb) {
    ...
    var doTextReq = function(el) {
        console.log('Getting /element/' + el + '/text');
    ...
    if (typeof element === "undefined" || element == 'body' || element === null) {
        _this.element('tag name', 'body', function(err, el) {
            doTextReq(el);
        });
    } else {
        doTextReq(element);
...
webdriver.prototype.url = function(cb) {
    ...
    var req = http.request(
        _this.getOpts({url:'/url', method: "GET"}),
        function(res) {
            res.setEncoding('utf8');
            var data = "";
            res.on('data', function(chunk) {
