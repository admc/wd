...
{
  var configUrl = {};
  for (var key in remoteWdConfig) {
    configUrl[key] = remoteWdConfig[key];
  }
  ...
  for (key in defaults) {
    if (configUrl[key] === undefined) {
      configUrl[key] = defaults[key];
    }
  }
...
function remote() {
  ...
  var driverTypes = Object.keys(driverProtos);
  var driverType = driverTypes.indexOf(args[args.length - 1])>0 ? args.pop() : 'async';
...
module.exports = {
  ...
  getHttpConfig: function() {
    var cloned = {};
    for (var key in config.httpConfig) {
      cloned[key] = config.httpConfig[key];
    }
    return cloned;
  },
