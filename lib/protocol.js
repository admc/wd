...
protocol.eval = methodBuilder({
  method: 'POST'
  , relPath: '/execute'
  , cb: function(code, cb) { return callbackWithData(cb); }
  , data: function(code) { return { script: 'return ' + code, args: [] }; }
});

protocol.rawExecute = methodBuilder({
  method: 'POST'
  , relPath: '/execute'
  , cb: function() {
...
var executeJsScript = fs.readFileSync( __dirname + "/execute.js", 'utf8');
...
protocol.execute = methodBuilder({
  ...
  , data: function() {
    ...
    return {script: executeJsScript, args: [code, args]}; 
  }
});
...
// script to be executed in browser
var waitForConditionInBrowserJsScript = fs.readFileSync( __dirname + "/wait-for-cond-in-browser.js", 'utf8');
